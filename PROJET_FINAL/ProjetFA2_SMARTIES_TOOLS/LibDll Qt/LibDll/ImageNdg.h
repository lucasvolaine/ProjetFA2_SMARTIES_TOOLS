...
class CImageNdg {
	...
	///////////////////////////////////////
		...
		// constructeurs
        Q_DECL_EXPORT CImageNdg(); // par défaut
        Q_DECL_EXPORT CImageNdg(int hauteur, int largeur, int val=-1); // si -1 alors non pixels non initialisés
        Q_DECL_EXPORT CImageNdg(const std::string& nom); // fichier en entrée
        Q_DECL_EXPORT CImageNdg(const CImageNdg& im); // image en entrée
        ...
        Q_DECL_EXPORT void choixPalette(std::vector<int>& map) { // couleurs données par tableau de 256 couleurs RGB)
			for (int indice=0;indice<256;indice ++) {
