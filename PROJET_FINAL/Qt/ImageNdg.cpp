...
CImageNdg CImageNdg::transformation(const std::string& methode,int vMinOut, int vMaxOut) {
    ...
    std::vector< int > palette = this->lirePalette();
    out.choixPalette(palette ); // conservation de la palette
		...
		if (eltStructurant.compare("V4") == 0) {
			for (int i=1;i<agrandie.lireHauteur()-1;i++)
				for (int j=1;j<agrandie.lireLargeur()-1;j++) {
                    ...
                    out(i-1,j-1)= (unsigned char)qMin(minV4,(int)agrandie(i,j));
				}
		}
		else {
			...
			for (int i=1;i<agrandie.lireHauteur()-1;i++)
				for (int j=1;j<agrandie.lireLargeur()-1;j++) {
                    ...
                    out(i-1,j-1)= (unsigned char)qMin(minV8,(int)agrandie(i,j));
				}
			}
		}
	}
			...
			if (eltStructurant.compare("V4") == 0) {
				for (int i=1;i<agrandie.lireHauteur()-1;i++)
					for (int j=1;j<agrandie.lireLargeur()-1;j++) {
                        ...
                        out(i-1,j-1)= (unsigned char)qMax(maxV4,(int)agrandie(i,j));
					}
			}
			else {
				if (eltStructurant.compare("V8") == 0) {
				for (int i=1;i<agrandie.lireHauteur()-1;i++)
					for (int j=1;j<agrandie.lireLargeur()-1;j++) {
                        ...
                        out(i-1,j-1)= (unsigned char)qMax(maxV8,(int)agrandie(i,j));
					}
				}
			}
		}
