...
// Expansion et Egalisation Couleurs
...
if (methode.compare("complement") == 0) {
		...
		CImageNdg PlanR = this->plan(0);
		CImageNdg PlanV = this->plan(1);
		CImageNdg PlanB = this->plan(2);

		PlanR = PlanR.transformation("complement",vMin,vMax);
		PlanV = PlanV.transformation("complement",vMin,vMax);
		PlanB = PlanB.transformation("complement",vMin,vMax);

		out.miseAJourPlan(0,PlanR);
		out.miseAJourPlan(1,PlanV);
...
if (methode.compare("expansion") == 0) {
		...
		PlanR = PlanR.transformation("expansion",vMin,vMax);
		PlanV = PlanV.transformation("expansion",vMin,vMax);
		PlanB = PlanB.transformation("expansion",vMin,vMax);
...
if (methode.compare("egalisation") == 0) {
		...
		PlanR = PlanR.transformation("egalisation",vMin,vMax);
		PlanV = PlanV.transformation("egalisation",vMin,vMax);
		PlanB = PlanB.transformation("egalisation",vMin,vMax);
