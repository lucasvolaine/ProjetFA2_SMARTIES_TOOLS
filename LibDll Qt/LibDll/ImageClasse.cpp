...
/*CImageCouleur CImageClasse::affichage(const std::vector<SIGNATURE_Ndg>& tab, int R, int G, int B){
	...
	CImageClasse contours2(this->lireHauteur() + 2, this->lireLargeur() + 2);
	...
	for (int i = 1; i<contours2.lireHauteur() - 1; i++)
	for (int j = 1; j < contours2.lireLargeur() - 1; j++)
	if (contours2(i, j) != 0){
		out(i - 1, j - 1)[0] = R;
		out(i - 1, j - 1)[1] = G;
		out(i - 1, j - 1)[2] = B;
...
CImageClasse CImageClasse::filtrage(const std::string& methode, int taille, bool miseAZero) {
	...
	if (methode.compare("taille") == 0)
	{
		...
		if (miseAZero) {
			...
			std::vector<unsigned long> surface;
			std::vector<unsigned long> rayon;
			...
			for (int i = 0; i < this->lireNbPixels(); i++)
			{
				...
				int rayon = sqrt((int (surface[this->operator()(i)]) / 3, 14));
			}
...
std::vector<SIGNATURE_Forme> CImageClasse::signatures(bool enregistrementCSV) {
	...
	if (this->lireNbRegions() > 0) {
		...
		for (int i = 1; i <= this->lireNbRegions(); i++){
			for (int n = 0; n < 8; n++)
				tab[i].histoFreeman[n] = 0;//Initialisation à 0

			int norm = 0;
			int k = 0;
			while (k < tab[i].codeFreeman.length()){
				tab[i].histoFreeman[tab[i].codeFreeman.at(k) - '0'] += 1;
				norm += 1;
				k++;
			}

			for (int n = 0; n < 8; n++){//normalisation
				tab[i].histoFreeman[n] = tab[i].histoFreeman[n] / norm;
				//std::cout <<"Pour "<<n<< ": on a " << tab[i].histoFreeman[n] << std::endl;
			}
		}
		...
		for (int num = 1; num <= this->lireNbRegions();num++) {
			for (int code=0;code<(int)tab[num].codeFreeman.size();code++) {
				if ((tab[num].codeFreeman[code] == '0') || (tab[num].codeFreeman[code] == '2') || (tab[num].codeFreeman[code] == '4') || (tab[num].codeFreeman[code] == '6'))
					...
					tab[num].perimetre += (float)sqrt((float)2);	
			}
		}
		...
		//		{
		//			tab[num].perimetre += 1;
		//			int rayon = (tab[num].perimetre)/(2*3,14);
		//		}
		//		else
		//			tab[num].perimetre += (float)sqrt((float)2);
		//			int rayon = (tab[num].perimetre)/(2*3,14);
		...
		if (enregistrementCSV) {
		 std::string fichier = "../Res/" + this->lireNom() + "_SForme.csv";
			...
			if (!f.is_open())
				...
				f << "Objet; CG_i; CG_j; Surface; codeFreeman ; RE_Hi ; RE_Hj ; RE_Bi ; RE_Bj ; perimetre; distance i, distance j" << std::endl;
				for (int k = 0; k < (int)tab.size(); k++)
					f << k << ";" << tab[k].centreGravite_i << ";" << tab[k].centreGravite_j << ";" << tab[k].surface << ";" << tab[k].codeFreeman << " ; " << tab[k].rectEnglob_Hi << " ; " << tab[k].rectEnglob_Hj << " ; " << tab[k].rectEnglob_Bi << " ; " << tab[k].rectEnglob_Bj << " ; " << tab[k].perimetre << ";" << abs(tab[k].rectEnglob_Hi - tab[k].rectEnglob_Bi)/2 << ";" << abs(tab[k].rectEnglob_Hj - tab[k].rectEnglob_Bj)/2<< std::endl;
			}
			f.close();
		}
	}
...
CImageClasse CImageClasse::distance(std::string eltStructurant, unsigned long valBord) {
	// distance au fond
	// gestion du bord : 0 ou autre valeur valant max des long
...
{
	// structure PIXEL utile pour empilements successifs	
	typedef struct {
		int abs;
		int ord;
	} PIXEL;
